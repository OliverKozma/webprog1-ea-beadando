...
case 'POST':
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO szoftver (nev, kategoria) VALUES (?, ?)");
        $stmt->execute([$data['nev'], $data['kategoria']]);
...
case 'PUT':
    try {
        ...
        $stmt = $pdo->prepare("UPDATE szoftver SET nev=?, kategoria=? WHERE id=?");
        $stmt->execute([$data['nev'], $data['kategoria'], $data['id']]);
...
case 'DELETE':
    try {
        ...
        $stmt = $pdo->prepare("DELETE FROM szoftver WHERE id=?");
